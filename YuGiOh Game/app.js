...
fetch(dataBase)
    ...
    .then(function(result) {
        ...
        playerField.addEventListener('click', e => {
            playerFieldChildren.forEach(child => {
                if (child.getElementsByTagName('img').length === 0) {
                    e.target.innerHTML = `<img src="${selectedImage[0].src}" alt="" class="pic">`
                }
                if (child.getElementsByTagName('img').length > 0) {
                    // console.log("no");
                }
            ...
            for (let i = 0; i < 5; i++) {
                if (parseInt(playerHandandDeckChildren[i].innerHTML.replace(/\D/g,'')) === parseInt(selectedImage[0].src.replace(/\D/g,''))) {
                    playerHandandDeckChildren[i].innerHTML = '';
                }
            }
            // Loop through the hand and take it from the hand and into the "inPlay" array. 
            playerFiveCardHandObjects.forEach(card => {
                if (card.id === parseInt(selectedImage[0].src.replace(/\D/g,''))) {
        ...
        playerField1.addEventListener('click', e => {
            if (playerField1.getElementsByTagName('img').length > 0) {
